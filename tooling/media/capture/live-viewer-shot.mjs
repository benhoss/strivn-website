...
//   3. waits for the offline queue to sync, then backdates the run ~41' and
//      spreads the goals (14' / 27' / 38') so the page reads mid-match
...
const OPPONENT = 4; // opponent_teams: FC Aurore (team 1)

// 1 ── the match event (starts "50 minutes ago" in the team's wall-clock)
const eventId = firstInt(psql(`INSERT INTO calendar_events (team_id, title, start_at, end_at, event_type, opponent_team_id, is_home, created_at, updated_at)
...
try {
  ...
  await browser.close();
  // 5 ── leave no trace: drop the match event (cascades run/moments/periods)
  if (!process.env.KEEP_MATCH) {
    psql(`DELETE FROM calendar_events WHERE id=${eventId};`);
    console.log('cleaned up match event', eventId);
  } else {
    console.log('KEEP_MATCH set — event', eventId, 'left in place');
  }
}
